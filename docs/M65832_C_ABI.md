...
| **B** | Absolute base - typically 0 for flat memory model |
| **R29** | Frame pointer (when frame pointer is used) |
...
├─────────────────────────┤
│   Saved frame pointer   │  ← Old R29 (if frame pointer used)
├─────────────────────────┤ ◀── Frame Pointer (R29)
│   Callee-saved regs     │  ← R16-R23, R48-R55 if used
├─────────────────────────┤
│   Local variables       │
├─────────────────────────┤
...
|----------|---------------|
| Local variables | `SP + offset` or `FP - offset` |
| Incoming stack args | `FP + offset` (positive offset from FP) |
...
| `float` | 4 bytes | R0 (bit-cast) |
| `double` | 8 bytes | R0 (low), R1 (high) |

### 7.2 Aggregate Return Values
...
Callee-saved: R16-R23, R29, R48-R55
Reserved:     R24-R28, R31, R56-R63
Arguments:    R0-R7
