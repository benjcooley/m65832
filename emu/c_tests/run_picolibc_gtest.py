...
def find_test_files() -> List[Tuple[str, str, str]]:
    ...
    if Path(CUSTOM_TESTS).exists():
        for f in sorted(Path(CUSTOM_TESTS).glob("*.c")):
            desc = extract_description(str(f))
            tests.append(("baremetal", str(f), desc))
    
    # Top-level picolibc tests
    for f in sorted(Path(PICOLIBC_TEST).glob("*.c")):
        if f.name in ("lock-valid.c", "native-locks.c", "math_errhandling_tests.c",
                      "rounding-mode-sub.c", "try-ilp32-sub.c", "fma_vec.h",
                      "long_double_vec.h"):
    ...
    for subdir in subdirs:
        subdir_path = Path(PICOLIBC_TEST) / subdir
...
def compile_test(src_path: str, work_dir: str) -> Tuple[bool, str, str]:
    ...
    cmd = [
        CLANG, "-target", "m65832-elf", "-O0", "-ffreestanding",
        f"-I{SYSROOT}/include",
        f"-I{PICOLIBC_TEST}",
...
def run_single_test(suite: str, src_path: str, work_dir: str) -> TestResult:
    ...
    if name in skip_tests:
        return TestResult(
            name=name, suite=suite, passed=False, time_ms=0,
            skipped=True, skip_reason=skip_tests[name]
        )
    ...
    if not success:
        ...
        if "undefined" in err.lower() or "undeclared" in err.lower():
            return TestResult(
                name=name, suite=suite, passed=False, time_ms=elapsed,
                skipped=True, skip_reason="Missing symbols"
            )
        return TestResult(
            name=name, suite=suite, passed=False, time_ms=elapsed,
    ...
    if not success:
        ...
        if "undefined symbol" in err.lower():
            ...
            return TestResult(
                name=name, suite=suite, passed=False, time_ms=elapsed,
                skipped=True, skip_reason=f"Missing symbol: {sym}"
            )
        return TestResult(
            name=name, suite=suite, passed=False, time_ms=elapsed,
...
"""
    ...
    # Filter by suite
    if args.suite:
        all_tests = [(s, p, d) for s, p, d in all_tests 
                     if s.lower() == args.suite.lower() or 
                        s.lower().replace("_", "-") == args.suite.lower()]
    
    # Filter by specific test names
    if args.tests:
        test_names = [t.lower() for t in args.tests]
        all_tests = [(s, p, d) for s, p, d in all_tests 
                     if Path(p).stem.lower() in test_names or
                        any(Path(p).stem.lower().startswith(t) for t in test_names) or
                        any(t in Path(p).stem.lower() for t in test_names)]
    
    # Filter by pattern
    if args.filter:
        pattern = args.filter.replace("*", ".*")
        all_tests = [(s, p, d) for s, p, d in all_tests 
                     if re.search(pattern, Path(p).stem, re.IGNORECASE) or
